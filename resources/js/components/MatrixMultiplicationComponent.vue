<template>
  <div class="container my-5 bg-white">
    <h1 class="font-weight-bold text-center">MATRIX MULTIPLICATION TOOL</h1>

    <div class="row col-12">
      <div class="border col-12 mt-5 p-5 row shadow rounded-lg " v-if="resultingMatrix">
        <h5 class="col-12 mb-5 font-weight-bold text-center">RESULTING MATRIX</h5>
            ...
            <tbody>
              <tr v-for="(row, rowIndex) in resultingMatrix" :key="rowIndex" class="flex w-100">
                ...
                <td v-for="(column, columnIndex) in row" :key="columnIndex">
                  <div class="text-center">{{ row[columnIndex] }}</div>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div class="row">
      ...
      <div
        class="col-md-6 border col-md-6 p-3 rounded-lg shadow"
        v-for="(setup, index) in matrixSetup"
        :key="setup.id"
      >
        ...
        <div class="row">
          <div class="form-group col-12">
            ...
            <input
              type="number"
              class="form-control"
              :class="{'is-invalid': errors.has(index+'.columns')}"
              v-model.number="setup.columns"
              v-on:change="generateMatrix(setup)"
            />
          ...
          <div class="form-group col-12">
            ...
            <input
              type="number"
              class="form-control"
              :class="{'is-invalid': errors.has(index+'.rows')}"
              v-model.number="setup.rows"
              v-on:change="generateMatrix(setup)"
            />
                    ...
                    <td v-for="(column, columnIndex) in row" :key="columnIndex">
                      <div class="input-group input-group-lg">
                        <input
                          ...
                          v-model.number="row[columnIndex]"
                        />
                      </div>
                    </td>
                  </tr>
